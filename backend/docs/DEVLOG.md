...
新增 documents/full：从本会话的 documents.zip 读取选中文档目录下全部 Markdown 块，按数字路径顺序排序，保留 key 和正文以支持前端整篇阅读及引用定位。目录或块 key 均可选中文档；无归档或文档返回 404，越界路径返回 422。聚合在 document_view 中完成，不逐段发起 gRPC 请求，不混入其他文档。

先以 HTTP 回归测试复现缺少接口，再实现；backend 全量 96 项通过。真实归档中的 Orion 文档返回项目概览、时间预算和成功标准三个块，前端已验证整篇渲染和高亮。

## 2026-09-19 跨标签页会话目录

新增 GET /chat/sessions，直接调用持久化 CRUD 读取最近会话，不依赖浏览器本地缓存或已加载的 manager。limit 默认 100、限制在 1-200。先以第二个客户端重启后查询目录的测试复现 405，再增加接口；backend 全量 95 项测试通过。设计与 API 文档同步。
...
- 删除 Registry 在 complete、get_or_load 和加载完成时的服务关闭拒绝分支，以及对应 closed 字段；保留应用退出时的资源清理和 manager、订阅生命周期状态。
- 新增清理后重新加载会话的测试，先因服务关闭检查失败，删除后通过；backend 全量 65 项测试通过，设计与测试说明同步。

## 2026-09-12 23:21:24

### 已完成工作

- 删除 completion 的 request_id 字段、请求指纹计算和提交去重分支，turn.created 不再写去重元数据。每次 POST 按新提交处理，同一会话仍由 manager 限制一个活跃轮次。
- 同步 API、设计、表说明及对应测试文档；验证已删除字段被拒绝、相同问题独立提交创建不同会话，backend 全量 64 项测试通过。

### 当前边界

- 拿到 session_id 后断线使用 resume；首帧前未拿到会话 ID 时重新提交可能产生独立会话。

## 2026-09-12 23:13:01

### 已完成工作

- 删除 Registry.complete 的独立受理任务、shield 和配套清理集合，将 _complete 合并进 complete；请求协程直接执行校验、去重和创建逻辑。
- 调整取消测试：显式取消等待创建锁的请求会中断受理，不创建轮次；同步设计及对应测试文档，backend 全量 64 项测试通过。

### 当前边界

- TurnRuntime 仍独立执行，SSE 断开只解除订阅；请求协程显式取消时，不再保证整个受理过程继续完成。
