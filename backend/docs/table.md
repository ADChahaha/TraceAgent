...
`chat_events` 保存过程事件，供展示和 SSE 续传；`chat_messages` 设计为只保存可完整回传模型的稳定历史。业务接入后，普通完整消息单条提交；带工具调用的 assistant 与全部对应 tool 结果配齐后，在同一事务中一起提交。中断只丢弃尚未提交的组，不删除此前完整历史。

状态：schema 与 CRUD 已统一为 `chat_*`、`session_id`；会话函数为 `create_session/get_session/list_sessions/update_session`。service/routes 尚未迁移，仍使用旧 task 接口。初始化只创建当前表，不迁移或删除旧 `qa_*` 表及数据；旧库需重建。消息配对、取消交接及稳定历史业务仍待接入。
...
```text
backend 接收 agent event，并在 session runtime lock 内检查本地状态
  ...
  → 下一轮 _completion_messages 只按 sequence 读取 chat_messages
```
...
取消与组提交共用同一把 session runtime lock。当前事件若已进入临界区且使整组配齐，可先完成整组事务；取消先提交本地状态后，后续事件不能再写入。事务失败则整组回滚，不留下单独 assistant 或部分 tool 行。
...
旧轮组提交、取消和新轮启动共用 session 级锁：旧轮最后一次合法事务提交 → 本地取消事务提交 → 新轮保存用户消息并读取 chat_messages 快照 → 锁外生成。新轮不能在旧轮仍可写消息时提前读取历史。
...
| `sequence` | `INTEGER NOT NULL` | session 内递增序号，用于 SSE 续传。 |
| `event_type` | `TEXT NOT NULL` | 事件类型，例如 `session.created`、`agent.event`、`turn.completed`。 |
