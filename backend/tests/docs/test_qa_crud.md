...
这份测试验证 `backend/crud/crud.py` 的读写与 `backend/models/schema.py` 完全对齐：每张表的插入、查询、更新都严格使用当前表结构和列名，不再引用 `chat_documents`、`stage`、`metadata_json`、`user_message_id`、`chat_events` 等旧结构。
...
connect_database(tmp_path / "crud.sqlite3")
  -> initialize_database(connection)  # 按 SCHEMA_SQL 建出四张 Chat 表
  -> chat_crud.create_session / create_turn / create_message / list_turns / ...
  -> 断言返回 dict 的列集合恰好等于当前 schema 列
```

关键约定：

- `chat_sessions` 只有 `id/status/active_turn_id/created_at/updated_at`，无 `stage`、`metadata_json`、`error_message`。
- `chat_turns` 只有 `id/session_id/status/agent_completion_id/error/created_at/updated_at/completed_at`，无 `user_message_id`。
...
- `test_create_turn_writes_only_schema_columns`：建 turn 断言列集合恰为当前 schema 八列（含 `error`）。
...
- `test_create_turn_persists_error_column`：`create_turn` 与 `update_turn_status_if_current` 均可写入 `error`，读回一致。
