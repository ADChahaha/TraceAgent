...
- `test_runtime_cancel_before_first_step_cleans_up`：runtime 在 begin 事务前被取消，仍须自行收口（终态已定、done 置位），不留悬挂任务；write 回调抛错时降级为 failed 并经 fail 回调标记（测试用不可用的 write 回调模拟写库失败）。
