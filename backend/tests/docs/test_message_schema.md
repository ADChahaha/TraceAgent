...
- `test_message_table_rejects_invalid_rows`：拒绝非法角色、序号、组内位置、工具字段、JSON 及跨 task/turn 关联。
- `test_message_table_rejects_duplicate_position`：task 内消息序号和组内位置不能重复。
...
- `test_system_message_can_exist_without_turn`：系统消息可仅关联 task。
