...
- `test_delete_removes_raw_and_rebuilds_bundle`：删除调用 document service 时 files 为空、remove_raw 指向被删文件；删除后 bundle 引用不含该 raw；对缺失资源和不支持删除的 bundle 引用分别返回 NotFound 与 ValidationError。
