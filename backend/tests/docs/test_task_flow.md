...
- `test_create_task_commits_resolved_agent_fields_without_routing`：验证 `POST /tasks` 先返回入队状态，后台 pipeline 会给标准化 block 补 `document_id/block_id`，并在抽取完成后直接把 resolved 字段提交为 agent 来源；summary、result 和 audit 都不再输出 route、reviewed、review_decision 或 review_value。
