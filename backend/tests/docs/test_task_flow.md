...
TestClient 上传 PDF
  ...
  -> fake agent trace 或运行中的 source_indexed task event 里带 source_selectors，backend replay 用这张 path_id -> 原始 DOM id 映射改写 display_html 的可读 block id
  -> backend 要求 agent 终态 `result.fields[]` 和 `trace.fields[]` 使用 `field_name` 作为字段键；缺失时直接按 contract error 失败
  ...
  -> replay actions 优先从 trace.events 生成 model_message/tool 同一时间线；旧 trace 没有 events 时才回退到 trace.actions
```
...
- `test_create_task_commits_resolved_agent_fields_without_routing`：验证 `POST /tasks` 先返回入队状态，后台 pipeline 会给标准化 block 补 `document_id/block_id`，并在抽取完成后直接把 resolved 字段提交为 agent 来源；summary、result 和 audit 都不再输出 route、reviewed、review_decision 或 review_value，同时 replay 会把原文 HTML 的可读 block id 改成虚拟 path_id，向前端返回 path_id -> path_id 的 `source_selectors`，并剥掉 replay action 顶层 `reason`。
- `test_replay_uses_live_source_index_before_extraction_finishes`：验证终态 agent trace 尚未写入时，`GET /tasks/{task_id}/replay` 会使用最新 `source_indexed` 事件里的 `source_selectors` 改写 document_processor 的 display_html，让运行中的任务也能打开原文并高亮。
- `test_replay_actions_preserve_model_and_tool_timeline`：验证终态 replay 优先从 trace.events 生成 action 时间线，把非空 `model_message` 和 tool 完成事件按 seq 保留在原始相对位置，避免前端把所有工具折叠到文字上方。
- `test_replay_display_html_sanitizer_keeps_large_css_fast`：验证 replay 展示 HTML 的清理逻辑对大型普通 CSS 保持线性耗时，不会因为没有 page/footer 目标选择器而卡住 backend；同时继续删除页码、页眉、页脚相关 CSS 和 DOM。
...
- `test_agent_result_missing_field_name_returns_contract_error`：验证 agent 如果返回缺少 `field_name` 的终态字段对象，backend 会把任务标成 `failed`，并返回明确的 contract error，而不是把裸 `KeyError('field_name')` 透给前端。
