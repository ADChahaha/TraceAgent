    ...
    *,
    ...
    connection.execute(
        ...
        INSERT INTO field_routes (
            id, task_id, field_name, route, route_reason, needs_review, created_at
        )
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
    ...
    connection.commit()
    row = connection.execute(
        "SELECT * FROM field_routes WHERE id = ?",
        (route_id,),
    ).fetchone()
    field_route = row_to_dict(row)
    assert field_route is not None
    return field_route
...
def list_field_routes(
    ...
    rows = connection.execute(
        "SELECT * FROM field_routes WHERE task_id = ? ORDER BY created_at, field_name",
        (task_id,),
    ).fetchall()
    return [dict(row) for row in rows]
