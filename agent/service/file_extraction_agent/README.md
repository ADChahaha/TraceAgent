...
本包接收已准备的资源定位数组与完整历史消息，执行一轮模型/工具循环。`document_resources` 负责 HTML 转文件与文档 embedding 发布到 storage 服务；本包通过 storage 服务读取资源，把 `documents.zip` 解到内存虚拟文件系统、索引从独立对象读取，不导入资源生成包。
