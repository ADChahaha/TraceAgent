...
flowchart TD
    ...
    C --> D["返回 resource_path（归档与索引定位）"]
    D --> E["调用方保存路径"]
    E --> F["ChatCompletion: resource_path + messages"]
    F --> P["prepare 子进程: 拉取归档 + 校验索引 → workspace payload（父进程保存）"]
    P --> G["manager 创建问答模型、注册 CompletionRuntime"]
    G --> H["loop 绑定子进程工具与消息；graph 绑定执行函数并运行节点/路由；图内完整 messages 与重试状态"]
...
flowchart TD
    ...
    F --> G["executor 取消工具调用；run_operation finally kill 工具子进程"]
