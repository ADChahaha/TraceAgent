...
def build_resolution_messages(state: Any) -> list[Any]:
    system = SystemMessage(
        content=(
            ...
            "Null is a normal resolved value for fields or enum variants whose declared type is null. "
            "Do not use status failed just because the value is null, absent, negative, or an allowed enum variant represents no mention. "
            "Use status failed only when the field cannot be completed reliably and needs human review. "
            ...
            "Use update_soft_plan(plan) as a soft local work log. "
            "Each plan item should read like a compact stage title: what document area or evidence relationship you are checking, "
            "and which related field group may be resolved. Use statuses pending, in_progress, and completed. "
            "Start by using update_soft_plan to group fields by likely evidence topic or document area before detailed reading. "
            "Group fields of the same kind or fields that share part of the same evidence in the same plan item when they can be judged together. "
            "Name those related fields together in the plan step so the replay shows one work unit, not separate unrelated tasks. "
            "If a plan item mentions task fields, write each field name separately exactly as it appears in Task fields. "
            "Do not use ranges, through, all fields, remaining fields, or similar shorthand. "
            "Update that same plan item from in_progress to completed after the related fields are set; then move to the next grouped plan item. "
            "You may revise the grouping as you learn the document, but keep the plan close to those grouped evidence topics. "
            "Create or refresh the soft plan before starting a new local work unit and whenever the plan status changes enough to help replay. "
            "Treat each soft-plan item as a local work unit: read or preview evidence for that local scope, then write fields supported by that same scope. "
            "Stay close to the fields implied by the current plan item when calling set_field; if another field is genuinely related to the same evidence path, "
            "you may handle it, but do not let one plan item expand into the entire task. "
            "Before moving to a materially different topic, clause area, or field group, refresh update_soft_plan with the new current item. "
            ...
            "Plan grouping and evidence freshness are soft guidance for memory and replay clarity. "
            "Before calling finish, every soft-plan item must be completed. "
            "finish will reject pending or in_progress soft-plan items, so update_soft_plan with all items completed after all related fields are set. "
            ...
            "Do not write fields using only the overview or soft plan."
        )
    )
    human = HumanMessage(
        content="\n\n".join(
...
def _continue_instruction(state: Any) -> str:
    ...
    if _should_force_set_field_nudge(state):
        return (
            "You have read several pieces of evidence but have not written a field. Stop browsing across unrelated areas. "
