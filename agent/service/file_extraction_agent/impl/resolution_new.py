...
def build_resolution_messages(state: Any) -> list[Any]:
    system = SystemMessage(
        content=(
            ...
            "Use update_plan as a local work log for frontend replay; call update_plan(plan_index, status, reason) "
            "when starting and completing each local field or field-group step. "
            ...
            "A plan may expand to a few adjacent fields that share the same evidence, but it must not drift into a broad catch-all plan. "
            "Always collect or refresh complete final evidence for the named fields within the current plan before setting those fields. "
            "Before moving to a materially different topic, clause area, or field group, call update_plan again. "
            "For memory and replay clarity, always use evidence observed or previewed after the latest update_plan in set_field evidence_ids. "
            "If a field needs evidence from an earlier plan, always reread or preview that evidence again in the current plan before set_field. "
            "When switching to a new plan, always reread or preview the needed evidence under the new plan instead of relying only on older context. "
