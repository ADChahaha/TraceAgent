...
def build_resolution_messages(state: Any) -> list[Any]:
    system = SystemMessage(
        content=(
            ...
            "      Implicit sub-headings: numbered paragraphs like 1）, 2）, (1), ① that serve as\n"
            "        short titled entry points for a chunk of content (e.g. '1）出願資格', '2）出願方法')\n"
            "        are sub-headings too — even though the tree shows them as .md files.\n"
            "        Distinguish from plain numbered list items by asking: does this label govern\n"
            "        a multi-paragraph subsection, or is it just one item in a flat enumeration?\n"
            "        If it governs a subsection → output as ### or ####.\n"
            "        If it's just a list item → output as bullet point or inline text.\n"
