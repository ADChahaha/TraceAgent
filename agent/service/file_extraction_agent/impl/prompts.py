"""service.file_extraction_agent 的内部 prompt 组装层。"""
...
from service.file_extraction_agent.impl.block_ids import require_block_id, validate_block_ids
from service.file_extraction_agent.impl.schemas import EvidenceCollection, ExtractionInput
    ...
    return [
        {
            ...
            "content": (
                ...
                "每个 task field 只返回一个 FieldEvidence；"
                "即使字段 type=list 且证据来自多行，也要把证据合并到同一个 field_name；"
    ...
    return [
        {
            ...
            "content": (
                ...
                "如果 target_field.type=list，resolved value 必须返回字符串列表，不能用分隔符拼成字符串；"
