...
from service.file_extraction_agent.core.messages import build_qa_messages, visible_text
...
async def stream_qa_graph(
    *,
    ...
    """messages 通道输出可见增量 → updates 输出完整结果/重试 → 取消传播 → 关闭图流。"""
    graph = build_qa_graph(qa_model, tools, run_options)
