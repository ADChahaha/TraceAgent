...
`BASE_URL` 和 `OPENAI_API_KEY` 是模型服务连接参数。`RESOLUTION_MODEL` 用于字段抽取；字段抽取的 `REASONING_EFFORT` 默认是 `high`，可用于 DeepSeek 等 OpenAI-compatible 源显式指定思考强度，设为 `none/off/disabled/false/0` 时不传该参数。`THINKING_TYPE` 默认是 `disabled`，会通过 `extra_body` 显式关闭 DeepSeek thinking mode。`ROUTE_POLICY_MODEL` 只用于 route policy 阶段。route policy 不读取通用 `MODEL`，也没有默认模型名；缺少 `ROUTE_POLICY_MODEL` 时会返回 422。
...
- `base_url`、`api_key` / `openai_api_key`、`resolution_model_name`、`model`、`temperature`、`top_p`、`top_k`、`reasoning_effort`、`thinking_type`：可选，兼容形式的模型连接覆盖字段；未传 `model_config` 时会组装成同一份 `ModelConfig`。
