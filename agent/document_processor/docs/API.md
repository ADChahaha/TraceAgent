...
这份文档面向调用方，说明 `document_processor` 的 Python 入口、HTTP 入口、请求字段、返回结构和边界。它只描述对外稳定契约；内部 PDF / DOCX 解析实现见 [`DESIGN.md`](DESIGN.md)。

## 基本链路

`document_processor` 接收原始 PDF / DOCX 文件对象，输出统一的 Markdown 和 blocks。它不做字段抽取，也不负责给 blocks 生成业务稳定 `block_id`；如果后续要调用 `file_extraction_agent`，应由 backend 或 session 聚合层给每个 block 补稳定唯一 id。
...
```json
{
  ...
  "docling_artifacts_path": "/path/to/document_processor/impl/pdf/models/docling",
...
Python 入口返回 `document_processor.schemas.ProcessResult`，HTTP 入口返回同形状 JSON：
...
注意：`document_processor` 不生成 `block_id`。如果结果要进入 `file_extraction_agent`，调用方需要在 session 聚合阶段把每个 `ContentBlock` 转成 `file_extraction_agent.schemas.NormalizedBlock`，并补齐稳定唯一的 `block_id`。
...
如果调用方没有显式配置模型目录，运行时会把相关下载和缓存默认收口到 `document_processor/impl/pdf/models/` 下：
...
- 不做字段抽取。
- 不负责生成进入 `file_extraction_agent` 所需的 `block_id`。
