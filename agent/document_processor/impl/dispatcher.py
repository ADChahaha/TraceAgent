"""Map normalized file types to concrete document processors.

Purpose: keep file-type selection separate from per-format processing logic.
Input/Output: accepts a file object plus optional type and returns `ProcessResult`.
How to use: call through `document_processor.process(...)` or instantiate directly in tests.
"""
        ...
        match file_type:
            case FileType.PDF:
                processor_cls = _import_processor(
                    "document_processor.impl.pdf.processor",
                    "PdfProcessor",
                )
                return processor_cls()
            case FileType.DOC | FileType.DOCX:
                processor_cls = _import_processor(
                    "document_processor.impl.doc.processor",
                    "DocProcessor",
                )
