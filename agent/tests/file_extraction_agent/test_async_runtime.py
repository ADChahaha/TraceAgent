...
def test_async_stream_preserves_events_and_cleanup(resource_path, monkeypatch, ending):
    ...
    monkeypatch.setattr(runtime_module, "stream_completion_events", lambda **kw: async_items([
        {"type": "model_message", "content": "答案"},
        {"type": f"completion.{ending}", "status": ending},
    ]))
...
def test_waiting_streams_leave_executor_free_and_cancel_cleanly(resource_path, monkeypatch):
    ...
    async def run():
        ...
        streams = [manager.create(completion_id=f"async{i}", resource_path=resource_path,
                   messages=[DocumentQaMessage(role="user", content="问题")]) for i in range(4)]
        tasks = []
        try:
            ...
            assert (await asyncio.wait_for(tasks[0], 1))["type"] == "completion.cancelled"
            for task in tasks[1:]:
                task.cancel()
