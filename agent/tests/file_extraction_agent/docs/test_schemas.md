...
```text
backend 传入 completion_id + resource_path + messages
...
- `test_completion_request_accepts_resource_path_and_append_only_messages`：验证 completion request 可以接收资源路径和 append-only 历史消息，并且对象上不再存在 memory 字段。
