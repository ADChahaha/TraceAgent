...
documents(filename + html)
  ...
  -> 只为 paragraph/list/table 可读叶子节点从原始 HTML 节点读取 id / data-element-id，生成 path_id -> 原始 DOM id 的 source_selectors
...
- `test_source_selectors_map_readable_path_ids_to_original_dom_ids`：确认只有可读叶子 `path_id` 能通过 `source_selectors()` 映射回原始 HTML 的 DOM id，document/section 目录不会进入映射。
- `test_document_direct_blocks_use_document_root_namespace`：确认没有 section 包住的文档直下 paragraph/list/table 会进入 `0001.0000.xxxx` 命名空间，不和真实 section 目录同级抢编号。
