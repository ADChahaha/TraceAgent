...
documents(filename + html)
  ...
  -> 给每个虚拟节点生成 0000.0001 形式的裸 path_id
  -> 从原始 HTML 节点读取 id / data-element-id，生成 path_id -> DOM id 的 source_selectors
...
- `test_source_selectors_map_path_ids_to_original_dom_ids`：确认虚拟 `path_id` 能通过 `source_selectors()` 映射回原始 HTML 的 DOM id，供 replay 原文定位使用。
