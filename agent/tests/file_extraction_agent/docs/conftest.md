...
`resource_path` 将测试 HTML 交给真实资源准备流程（publish_resources 显式传入 store 与桶名），注入三维向量替身和字符 tokenizer，把产物发布到测试专用 storage 服务的一个独立随机桶（session 级 `storage_server` fixture 在随机端口启动），返回资源定位数组 `[{type, location}]`。每个测试隔离目录、配置和桶名，不加载真实 embedding 模型。
