...
这份测试覆盖新 resolution prompt 和工具暴露顺序。模型应围绕虚拟文件树浏览材料，用 `reason` 解释用户可见动作，只要看到自己认为可能是字段证据的文本、列表项或表格行，就用 `bind_evidence` 立刻绑定 selector，不等字段值最终确定；如果某字段已有候选证据，写字段前必须用 `review_field` 复看这些候选证据；没有候选证据的字段不需要空 review。`write_field` 通过 `final_evidence` 提交复看后保留的最终证据，而且只能保留真正有用的 selector，丢弃只是同主题、背景、重复或弱相关的候选证据；prompt 还说明只有 `null` 类型字段或 `null` enum variant 可以空 `final_evidence` 提交。
