...
当前候选证据动作叫 `add_candidate_evidence`，强调它只是宽松的候选笔记：只要某个显式 `evidence://` block link 可能相关，就可以随时保存；一次只记录一个字段和一个 paragraph/list/table block。`review_evidences` 再把 block 展开成 inline link，让模型判断当前候选是否足够写入或还需要继续找证据。普通 `tree` 导航、调用 `read` 前、常规候选保存、重复相邻读取和纯 bookkeeping 都可以留空；`read` 返回后如果读到有意义内容，下一轮用 `task_spec` 主语言自然一句话概括“看了什么、是否有价值”，证据 quote 保留原文语言；如果这个概括依赖同一 section 里的连续 block，就用一个 `evidence://range/<start>/<end>` 覆盖整段连续 block，而不是只链第一个 block。`review_evidences`、`write_field`、`submit_result` 这类判断点才更常输出。可见文字不使用 `Read / Finding / Next` 等固定小标题，也不直接叙述工具名。如果引用原文，必须写成 `["原文短语"](evidence://...)` 这样的 Markdown evidence link，不能只用裸引号。
...
- `test_resolution_messages_describe_candidate_policy_without_tool_manual`：确认 system prompt 只保留高层规则；assistant content 是按最新观察决定是否输出的用户进度更新，不是逐工具日志；机械动作和普通候选保存可以留空，read 有价值时用 `task_spec` 主语言自然短句概括，同时确认引用原文必须使用 Markdown evidence link 且证据 quote 保留原文语言。
