...
- `test_resolution_messages_embed_compact_document_outline`：确认 resolution prompt 包含 compact outline、字段一次性定案规则、reason 面向用户和尽量使用文档语言的约束、replay 同步约束、section/同层 block 读取策略、inline/table row/list item 证据粒度要求，并把具体工具参数与读取行为交给 LangGraph 绑定的 tool docstring 注入；同时确认不再内嵌 `query_audit` 相关提示。
