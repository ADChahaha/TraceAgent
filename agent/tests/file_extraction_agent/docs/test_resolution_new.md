...
当前候选证据动作叫 `add_candidate_evidence`，强调它只是宽松的候选笔记：只要某个显式 `evidence://` block link 可能相关，就可以随时保存；一次只记录一个字段和一个 paragraph/list/table block。`review_evidences` 再把 block 展开成 inline link，让模型判断当前候选是否足够写入或还需要继续找证据。模型第一轮工具调用前必须先用 `task_spec` 语言写一句简短开场，说明要抽取哪些字段；之后才按动态内容规则决定是否说话。普通 `tree` 导航、调用 `read` 前、常规候选保存、重复相邻读取和纯 bookkeeping 都可以留空；`read` 返回后如果读到有意义内容，下一轮用 `task_spec` 主语言自然一句话概括“看了什么、是否有价值”，证据 quote 保留原文语言；如果读到的内容很快会进入字段定案，read 概括只负责给 reviewer 定位，不提前完整写最终结论。模型应尽量按紧凑字段决策簇推进：`read` 相关 block 后，保存候选、复核证据并写入当前字段或语义块，再切到无关阅读；当 review 已经足够时，下一轮通常直接写同一字段。`review_evidences` 在下一步显然是 `write_field` 且事实已在 read 后概括过时可以静默，`write_field` 只做增量定案说明，避免把同一组原文事实完整重复两遍。如果 read 概括依赖同一 section 里的连续 block，就用一个 `evidence://range/<start>/<end>` 覆盖整段连续 block，而不是只链第一个 block。需要更细粒度引用时，paragraph 句级用 `evidence://<block>/Sxxx`，list item 用 `evidence://<block>/Ixxx`，table row 用 `evidence://<block>/Rxxx`。可见文字不使用 `Read / Finding / Next` 等固定小标题，也不直接叙述工具名。如果引用原文，必须写成 `["原文短语"](evidence://...)` 这样的 Markdown evidence link，不能只用裸引号。
