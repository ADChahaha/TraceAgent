...
```text
GraphState
  -> broad prompt 提供所有字段、pending_fields、可搜索内容摘要、全字段候选池和工具结果
  -> resolution prompt 提供所有字段、pending_fields、全字段候选池、已完成字段和工具结果
  -> 在 payload.tool_contract 中注入每个 action 的用途、入参、返回和约束
  -> search_grep 的 query 统一要求使用大写 OR 连接短关键词
  -> broad 明确注入 copy_field_candidates，且不暴露 count_field_candidates
  -> resolution 明确注入 count_field_candidates 和 add_resolution_candidate(values) 的契约
...
- broad prompt 会带上 task、全字段定义、metadata、搜索索引摘要和全字段候选池。
- broad 和 resolution prompt 都会暴露统一的 `search_grep` 和精确工具契约。
- broad prompt 额外暴露 `copy_field_candidates`，resolution prompt 额外暴露 `count_field_candidates`。
- prompt 会明确要求 search query 使用 `term1 OR term2 OR term3` 固定格式。
- resolution prompt 会携带全字段候选池摘要，不直接携带原始 blocks。
...
- 构造一个字段和一个段落索引。
- 确认 broad prompt 明确要求 `BroadAction`、`search_grep`、`copy_field_candidates` 和 `finish_broad`，并说明 broad 不暴露 count。
- 确认 system message 明确 `OR` 查询格式。
- 确认 payload 包含全字段列表、pending_fields、metadata、可搜索内容摘要和 `tool_contract`。

`test_build_resolution_messages_includes_candidate_pool_and_prior_decisions`

- 为目标字段写入一个候选。
- 确认 resolution prompt 包含候选 `candidate_id/text`。
- 确认 resolution prompt 暴露 `search_grep`、`count_field_candidates` 和 `final_decision`，且不暴露 `copy_field_candidates`。
