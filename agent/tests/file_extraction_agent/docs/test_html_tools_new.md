...
这份测试覆盖新抽取工具层的 read 后强制判断状态机。工具围绕虚拟文件树工作：`read` 只负责打开一个 paragraph/list/table 对象；成功读取后必须立刻用 `bind_evidence` 把当前对象作为字段候选 block evidence，或用 `skip_read` 标记当前对象完全无关。字段写入前必须紧跟一次同字段 `review_evidences`，由工具把候选 block 展开成 Sxxx/Ixxx/Rxxx inline selector；`write_field(final_evidence=...)` 只能使用这次 review 返回的 inline selector，不能直接写 block selector。
...
documents + task_spec
  ...
  -> 紧跟 write_field 写入字段值和刚 review 的 inline final_evidence
...
- `test_write_field_requires_reviewed_inline_evidence_not_block_evidence`：确认字段写入必须紧跟同字段 review，且 `final_evidence` 只能使用 review 返回的 inline selector，不能使用 block selector 或未 review 的编号。
- `test_write_field_requires_immediately_preceding_same_field_review`：确认同字段 review 一旦被其他工具调用隔开、review 了其他字段，或最后一次 review 失败，就不能直接写字段。
- `test_missing_and_null_enum_values_can_use_empty_evidence_after_review`：确认 missing 字段和 null enum variant 可以使用空证据，但也必须先紧跟同字段 review。
