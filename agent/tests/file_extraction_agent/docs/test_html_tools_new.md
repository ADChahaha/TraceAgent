...
这份测试覆盖 QA completion 的模型工具集合。工具不再写字段，而是模仿 code agent 阅读项目的方式，在多文档 virtual tree 上执行 `tree / grep / read / inspect`。
...
- `test_module_exports_qa_helpers_only`：验证模块公开 helper 已切换到 `_tree/_grep/_read/_inspect`。
