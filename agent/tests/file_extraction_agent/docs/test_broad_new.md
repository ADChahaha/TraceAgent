...
- `test_build_broad_messages_includes_task_and_tree`：确认 broad prompt 包含任务字段、文档树、完整 HTML、resolution 工具名清单（包含离散读取 `read_blocks` 和连续读取 `read_block_range`），并说明精确工具参数来自 resolution 阶段绑定的 tool docstring；同时覆盖大表 `SELECT *` 限制、query audit 说明、禁止预填最终答案，以及不再包含任务特化示例。
