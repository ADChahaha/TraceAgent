...
`service.document_processor.schemas` 这一层本质上是在定义文档标准化链路里的公共数据契约，不做业务流程控制，也不直接依赖 FastAPI 之类的协议层。它主要做三件事：
...
一句话：这个文件用来钉住 `service.document_processor.schemas` 的数据结构，不让后面改着改着把返回格式改坏。
...
`service.document_processor` 后面会被 route 和抽取流程一起消费。这个测试文件先把最基础的 schema 契约固定住，后面继续补 `processor.py` 的注册式入口和具体处理器时不容易把输出结构带偏。
