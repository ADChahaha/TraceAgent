...
MinerU pages
  ...
  -> 正文小标题在 HTML 中输出为普通 `<p>...</p>`，不生成 section，也不会进入 Contents 的 outline_tree
  ...
  -> Markdown 标题只让全局高置信 h2 聚类簇进入 heading，其余 title 降级成普通正文行
  -> `1）` / `【注意】` / `<<注意>>` / 括号编号 / 单字母标号这类正文里的小标题会降级成普通正文行
  -> `2.日程` 这类 ASCII 点后没有空格的紧凑编号 title 会降级成普通正文行
  -> 短的独立 `1．` / `2．` 编号 paragraph 也会作为正文小标题候选降级成普通正文行
  -> MinerU 误标成 title 的提出期限/締切日期提示行会降级成普通正文行
...
- `test_build_html_renders_body_subheadings_without_section_nodes`：确认正文里的 `1）`、短 `1．` 编号项和小字号 `3．` 标题只渲染为普通段落，不会生成 heading 或 section 节点。
...
- `test_build_markdown_demotes_deadline_title_to_body_line`：确认带日期和时间的提出期限提示行即使被 MinerU 标成 `title level=2`，也只作为普通正文行输出，不会变成 Markdown heading 或 HTML section。
- `test_build_markdown_demotes_compact_numbered_title_with_ascii_dot`：确认 `2.日程` 这类 ASCII 点后没有空格的紧凑编号 title 会降级为普通正文行，同时不影响英文 `1. Definitions` 这类正式标题。
- `test_build_markdown_uses_global_h2_layout_band_and_bolds_other_titles`：确认同一文档内按 title 版面特征识别高置信 h2 簇，高档章节进入 `##`，其他 title 降级为普通正文行。
- `test_build_markdown_clusters_h2_band_with_width_and_indent_features`：确认 h2 识别不只看高度，也会利用宽度、缩进、字符数和行数，把版面相近但语义不同的标题分开。
- `test_build_markdown_demotes_angle_bracket_title_after_agglomerative_clustering`：确认层次聚类会把小字号全角尖括号局部标题降级为普通正文行，同时真正章节标题仍输出为 `##`。
