...
RoutePolicyInput(task_spec + field_outputs + refs_with_text + field_processes)
  ...
  -> 校验每个待评估字段都有对应 RouteFieldProcess
  -> 返回 ValidatedPolicyInput(field_definitions_by_name / field_outputs_by_name / refs_by_field_name / processes_by_field_name)
```
...
- 输入一个合法字段和一条证据 ref。
- 同时传入 broad / resolution 过程摘要。
- 确认 validator 返回后能按 `invoice_no` 直接取到定义、输出、证据和值，以及 search 查询词。
...
`test_validate_route_policy_input_requires_process_group_for_every_field_output`

- 删除 `field_processes` 分组。
- 确认每个待评估字段都必须带两阶段过程摘要。
