...
def test_wheel_contains_resource_and_qa_modules(tmp_path):
    ...
    assert {
        "service/file_extraction_agent/core/loop.py",
        "service/file_extraction_agent/core/tools/embedding.py",
        "service/file_extraction_agent/core/graph.py", "routes/file_extraction_agent.py",
    ...
    assert "document_main.py" not in names
    assert not any(name.startswith(("service/document_processor/", "service/document_resources/")) for name in names)
    assert "routes/document_resources.py" not in names
    assert "service/file_extraction_agent/turn_stream.py" not in names
