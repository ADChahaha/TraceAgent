...
这份文档面向调用方，说明 `file_extraction_agent` 的 Python 入口、HTTP 入口、请求字段、返回结构和 trace 语义。它只描述对外稳定契约；内部 broad / resolution / tool 的实现细节见 [`DESIGN.md`](DESIGN.md)。

## 基本链路

`file_extraction_agent` 不接收原始 PDF / DOCX，也不负责 OCR。调用方必须先把文档处理成标准化 blocks，再把 blocks 和显式 `task_spec` 交给本包：

```text
backend / document_processor 产出标准化 blocks
