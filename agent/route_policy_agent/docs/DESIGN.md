...
这份文档记录 `route_policy_agent` 的设计。它是 `agent service` 下独立于 `file_extraction_agent` 的第三个处理阶段，负责像第三方评价者一样，根据任务、字段输出和 refs 中的证据文本判断字段结果应当 `accept / review / reject`。

## 目标与边界

`route_policy_agent` 的目标不是重新抽取字段，而是判断 `file_extraction_agent` 已经产出的字段结果能否进入后端治理流程的下一步。
...
- 不接收 PDF / DOCX 原始文件。
- 不读取完整原文，也不重新跑 document_processor。
...
这层和 `file_extraction_agent` 的区别是：
...
`route_policy_agent` 的输入应当只包含做 route 判断所需的信息。第一版把它设计成第三方评价者：除任务/字段定义和待评估字段输出外，只看 `refs` 中携带的证据文本和来源位置。
...
backend 传入 task_description / task_spec
  ...
  -> route_policy_agent.schemas 做 Pydantic 解析
...
`backend` 不做 LLM route 判断，只保存 `route_policy_agent` 的输出并驱动状态流转。`backend` 可以做必要的数据库完整性校验，但不重新解释字段证据。

## 与 `file_extraction_agent` 的关系

`file_extraction_agent` 应保持输出 `ExtractionResult(result + trace)` 的职责，不内置 route policy。
...
- 把 route policy 独立出来，可以避免 `file_extraction_agent` 同时承担抽取、证据留痕和业务放行判断。
