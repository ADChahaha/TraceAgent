...
- completed work:
  ...
  - 同步更新 DESIGN.md / README.md / API 与测试文档、测试 import 与打桩路径，以及 CI 里 `document_service.document_processor.mineru_converter` 的 import。
- current progress:
...
- next step:
  - 可评估是否在 `document_service/document_processor/__init__.py` 再导出 `process` 作为包级入口，进一步收敛对外 API。
...
- completed work:
  - 清理 `routes/document_processor.py` 的边界依赖：route 层改为从公开 `document_service.document_processor.processor` 导入 `InvalidFileObjectError`，不再引用 `document_service.document_processor.impl.base`。
