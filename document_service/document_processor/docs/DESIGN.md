...
`document_service.document_processor` converts uploaded documents into HTML that is usable
...
document service 的 gRPC 入口为 `DocumentResourceService.PrepareResources`：先调用本模块的 `processor.process(...)` 解析，再由同级 document_resources 生成文档树和索引，返回资源路径。本模块不单独暴露网络解析入口。
...
```text
document_document_service/document_processor/
├── __init__.py
├── processor.py
├── schemas.py
├── pdf/
