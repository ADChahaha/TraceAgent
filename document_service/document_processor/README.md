...
`document_service.document_processor` receives PDF or DOCX file objects and returns a
...
```python
from document_service.document_processor.processor import process
...
document service 的 gRPC `DocumentResourceService.PrepareResources` 会先调用本模块解析 PDF/DOCX，再建立资源目录和索引；本模块的 Python 接口不变。
