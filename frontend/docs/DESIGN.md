...
浏览器打开 /
  ...
  -> ReplayReview 展示文档 HTML、虚拟文件树、工具调用时间线和字段写入动作
...
/tasks/{task_id}
  ...
  -> 右侧不再渲染逐块工具时间线、backlog 面板或底部 dialogue；左键点击文字流继续下一步，hover 某条 action 时才显示“跳到这一步”和“只播放这一步”的小按钮
  ...
  -> 右侧 reason 是普通正文段落；工具调用采用 Codex 风格的灰色运行记录行：read/read_element/read_section 用搜索图标，其余工具用终端图标，图标后直接是一句 `Ran/Read/Searched/Queried ...` 短摘要和少量 meta，不再拆出 tool_name 列、参数 chips、诊断卡或校验错误正文
  ...
  -> write_field action 展示字段写入卡：field_id/display_name、status、value、final_evidence、route badge 和 route_reason；它替代旧 set_field 展示
  ...
  -> 如果没有 write_field action 但存在 needs_review 字段，replay 末尾显示同样的字段卡，并提示等待人工补录
