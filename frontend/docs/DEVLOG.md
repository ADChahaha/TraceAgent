...
### 恢复原有补传入口并避免选择文件时的刷新冲突

按用户要求移除聊天区新增的附件按钮，继续使用 Sources 中原来的 Add files。检查指定会话确认服务端处于 ready，未运行回答。组件测试复现文件选择器返回时 focus 刷新会话的冲突：连接进入 loading 后原文件输入被禁用，选中事件可能被 addFiles 的 unavailable 校验丢弃。现在打开原选择器期间暂停焦点和定时刷新，选择或原生 cancel 事件后恢复；不增加新的可见入口。

TDD 先确认聊天存在多余按钮、选择期间发生恢复请求而失败，再移除按钮并跟踪选择器状态。回归覆盖原入口对话后补传、草稿与历史保留、运行状态、选择/取消后的同步。72 项测试、ESLint 和生产构建通过，3000 服务已更新。


### 对话后从聊天输入区添加文件

此前补传入口只在 Sources，阅读引用后入口被阅读器替换，聊天输入区没有附件入口。现在输入框下固定提供 Add files，复用当前 session 的逐文件上传队列；选文件立即返回 Sources 显示进度，保留历史和草稿，处理完成后继续追问。附件禁用条件与发送分开，空资料会话仍能添加文件，回答运行时不能修改资料。

真实浏览器同时发现 801-1023px 窗口内旧全局 CSS 隐藏拖动条，聊天区误占仅 20px 的网格列。工作台显式设置拖动条 inline-flex，保持三列占位，800px 以下仍走 Documents/Chat 切换。

TDD 先验证缺少附件入口和拖动条占位而失败，再补实现。覆盖回答后阅读引用时补传、同一 session、进度、历史与草稿保留、继续追问及运行/空资料状态。71 项前端测试、ESLint 和生产构建通过。服务已重启；真实浏览器在已有历史且来源为空的会话确认聊天区正常显示，输入框下 Add files 可见且可用。上传和追问行为由模拟 API 的组件测试验证，未向用户会话写入测试文件。

## 2026-09-19

### 移除来源列表的文档下拉框

移除 Sources 列表底部的 Open a document 下拉框及专用样式；引用仍可打开原文，阅读器内仍可切换文档和关闭。回归测试先因下拉框仍存在而失败，移除后通过；全部 68 项测试、ESLint 和含 TypeScript 检查的生产构建通过。

检查 Remove 请求链路发现：document service 在删除后仍有文件时会重新解析全部剩余原文件，并重建文档归档和 embedding 索引，DELETE 等待发布完成，前端再刷新会话。此次只记录代码层面的耗时来源，未测量各阶段时长，也未修改删除服务逻辑。

### 移除输入框下方的建议问题

首页和会话页不再渲染三个胶囊形建议问题按钮，删除共用建议组件及专用样式。首页概览中的摘要和关键问题入口继续填入可编辑草稿，输入、发送及暂停行为保持不变。

补全工作区已有的相关测试，先确认首页和详情因仍显示建议按钮而失败，再移除渲染并通过 20 项定向测试；清理停用代码后，全部 68 项前端测试、ESLint 和含 TypeScript 检查的生产构建通过。同步设计和两份测试说明。

### 原文阅读替换 Sources 并可关闭

左栏默认仅显示 Sources，不再把来源和正文上下堆叠。选择文档或点击引用后，阅读器占满左栏，顶部保留文档选择器和关闭按钮；关闭清空选择并恢复 Sources，再次点击同一引用可重新打开。减少正文尾部空白，引用段落改为淡黄色高亮，深色主题对应深金色背景。

先新增 Sources/阅读互斥和关闭回归测试，确认旧页面默认就渲染 Source content 而失败，再实现双视图切换；同步调整原先假定默认加载正文的用例。前端 68 项测试、ESLint、TypeScript 和生产构建通过。真实浏览器验证引用 4 打开整栏阅读、关闭返回 Sources；对照实际下载的 Orion DOCX 的 5 段标题、章节和正文，页面全部包含，只有预算段落被高亮。服务已更新。

### 引用标记缩小并改黑色

按反馈将数字引用从 19px 蓝色圆标改为 14px 黑底白字，字号由 11px 降至 9px；悬浮使用轻微灰色变化与 1px 轮廓。仅调整视觉样式，引用定位行为保持不变。ESLint、TypeScript 和生产构建通过。
