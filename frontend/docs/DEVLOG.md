...
### 整篇 Markdown 原文与引用可见性

原文面板此前把归档中的单个段落当作整份文档，点击引用又用该块替换内容。现按一级文档目录分组，通过 documents/full 一次读取所有 Markdown 块，按原顺序连续渲染，并从目录层级恢复章节标题。引用只确定文档和高亮块，定位后上下文保留；相同资源版本的轮询不重新加载正文。

引用数字原样式依赖未定义的 replay-accent，浏览器实际计算为透明背景加白字。工作台为数字标记明确设置强调色背景及对比文字，悬浮只增加轮廓；浅色和深色模式均实际检查未悬浮颜色。

TDD：整篇接口先返回 404，前端新测试先因缺少完整上下文失败；实现后验证全文、标题、列表、同文档高亮切换及跨文档加载。前端 67 项、backend 96 项测试通过，ESLint、TypeScript 和生产构建通过。真实浏览器对已有中文回答点击引用 4、5，均保留 Orion 的三个块，分别只高亮时间预算与成功标准段落；默认状态的数字 1-5 均可见。正常服务已重启到新版本。

### workspace 跨标签页与跨浏览器

此前目录只存在 localStorage，且上传完成后仍停在根地址，另一个标签页无法可靠发现或复制同一个 workspace。现在菜单从 GET /chat/sessions 读取实际数据库的目录，打开菜单、重新聚焦或本页会话变更时刷新，失败明确提示。移除本地会话列表事实来源。

首页文件全部处理完成后自动进入 /tasks/{session_id}，通过页面内草稿映射保留已输入但未发送的问题，不把草稿写入 URL。空闲详情页每 5 秒及重新聚焦时 GET resume 恢复最新资源和对话；本页上传或回答中不打断操作。

TDD 先验证空缓存目录、上传后导航和焦点恢复三项失败，再实现。前端 65 项测试、ESLint、TypeScript 和生产构建通过。真实浏览器同时打开 127.0.0.1:3000 与 localhost:3000：两者列出并打开相同已有 workspace；新上传后地址自动成为 /tasks/062bc5013ea74775a1acd45594d44f6f，草稿保留，另一个地址的标签页从菜单找到该新会话并读到同一文件。第二个标签页补传第三份文件后，第一个标签页未手动刷新，来源数量自动从 2 变为 3，验证了定期同步。服务继续使用实际数据库和正常启动脚本。
