...
读取 globals.css
  ...
  -> 检查 Agent 阅读列是否使用 `blank / min+optional text / blank`，保证宽度变窄时先压缩左右空白，再压缩可选文字宽，最后停在最小文字宽
...
- `Agent 阅读列按最小文字宽、可选文字宽、空白的顺序收缩`：验证 Agent 中间文字框和输入框使用 `520px` 最小文字宽、`200px` 可选文字宽和左右 `1fr` 空白；容器变窄时先扣左右空白，空白归零后再扣可选文字宽，最后停在最小文字宽。
