...
任务详情页接收 task_id
  ...
  -> replay 工作台配色用 Codex 式 light/dark 主题承载三栏界面，tool 行默认淡化为无框文字，当前文件、hover action、阅读线和 evidence 高亮统一用中性灰，避免大面积蓝色块
  -> 顶部工具栏只保留单行任务上下文：左侧展示 `task_id / 当前选中文件名`，右侧只保留一个任务 status；多文件任务会按当前 action path 切换文件名
  -> iframe 内部把 backend display_html 包成白底全屏文档画布，并用 15px 系统 UI 字体统一标题、段落、列表和 booktabs 风格表格排版，不引入具体行业语义
  -> iframe 包装 display_html 时删除页码、页眉、页脚版本号等文档 chrome，避免 `Page 2` 和 `428249v2` 继续显示在中间画布
  -> result.fields 与 review.fields 合并成 replay 字段写入区
  -> 真实 file_extraction_agent 工具 tree/read/query_table/bind_evidence/review_field/write_field/submit_result 以 Codex 风格淡化运行文字行展示，reason 是普通正文，read 系列工具使用搜索图标，anchors 只服务左侧树和 HTML 动画、不进入右侧文字流
  ...
  -> 长字段值在字段写入区内部独立滚动，复核输入和提交按钮不被长列表挤出复核区底部
...
- `waiting_review 任务只展示 replay，并在 review 字段区里提交修正`：验证详情页不再出现“结果/复核/证据/审计”tab，也不展示原始 trace/audit 文案；当前 `set_field` 字段区显示字段显示名、`review` badge 和 route 原因，并能提交 `revise_and_approve` payload，刷新后同步最近任务缓存。
...
- `replay 工作台使用 Codex 式中性配色`：验证全局 replay palette 使用 Codex 式白/灰/黑主题变量，侧栏面板和工具行有明确灰阶变量，iframe evidence 高亮使用中性灰背景和边线，不再输出旧 teal 或大面积 cobalt 色值。
...
- `真实 file_extraction_agent 工具以 Codex 工具行展示`：验证 `tree/read/query_table/bind_evidence/review_field/write_field/submit_result` 使用真实 flat action contract；右侧 reason 是普通正文，tool 行是无边框、无底色的淡化一行摘要，`read` 使用搜索图标标记并显示 `Read paragraph Confidential` 这类语义文案，anchors 不进入右侧文字流，read/query/submit 的返回正文、Rxxx 和校验错误不进入右侧文字流。
- `replay agent 文字流和字段复核区不使用块状卡片样式`：验证 reason、tool 行和字段写入区都不再使用块状边框或灰底卡片；字段值改用系统 UI 字体和较小字号，避免右侧复核区出现等宽大字。
...
- `动作 result 的诊断内容不进入右侧文字流，字段写入区也不承接诊断文字`：验证 `*_audit.summary` 只作为工具 result 数据存在，不在右侧 agent 流里展开；后续 `set_field` 字段写入区也只展示字段值、路由和证据。
...
- `字段证据 chip 只定位文档证据，不回跳 replay action`：验证用户从字段写入区点击证据 chip 时，只暂停并滚动文档 iframe，不把 replay 序号切回最早引用该证据的 action。
...
- `必填字段没有 agent value 时在 replay 末尾显示空复核输入`：验证 agent 没有写入必填字段但 route_policy 要求 review 时，replay 仍会显示“等待人工补录”的字段写入区和空输入框，提交时把人工补录值发给 review 接口。
- `长字段写入区把字段内容和复核区分离，避免全屏时复核入口被撑出视口`：验证长列表字段写入时，字段值位于独立内容区，复核 textarea 和提交按钮仍在字段写入区的复核区中，不会混进长内容滚动区域；同时确认 replay 根节点带有字段写入布局状态，供全屏样式为底部复核区预留高度。
